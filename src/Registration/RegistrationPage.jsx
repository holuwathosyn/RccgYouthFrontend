...
const ChurchRegistrationForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ IMPORTANT: ONLY SUCCESS IF BACKEND CONFIRMS
      ...
      <div className={`relative w-full max-w-lg bg-white rounded-[32px] shadow-2xl p-8 border border-gray-100 transition-all duration-500 ${loading ? 'blur-[4px] pointer-events-none' : ''}`}>
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-3xl font-extrabold text-gray-900 text-center">
            <span className="text-red-600">RCCG</span> Glory of God
          </h1>

          <p className="text-gray-500 text-sm mt-1">
            Youth Bio Data Registration
          </p>

          <div className="mt-3 flex items-center gap-2 px-3 py-1 bg-gray-900 text-white text-[10px] font-bold uppercase rounded-full">
            <span className='animate-bounce'>
              <AlertCircle size={12} />
            </span>
            Strictly for Church Youth
          </div>
        </div>
...
const InputField = ({ label, ...props }) => (
  <div>
    ...
    <input
      {...props}
      required
      className="w-full px-4 py-3 mt-1 rounded-xl bg-gray-100 focus:ring-2 focus:ring-red-500 outline-none text-sm"
    />
  </div>
);
...
const PopupSuccess = () => (
  <div className="fixed top-10 left-4 right-4 z-[100] max-w-lg mx-auto">
    <div className="bg-white p-4 rounded-3xl shadow border border-green-200 flex items-center gap-4">
      ...
      <div>
        <h4 className="font-bold">Information Received!</h4>
        <p className="text-xs text-gray-500">Your details are saved.</p>
      </div>
    </div>
  </div>
);
